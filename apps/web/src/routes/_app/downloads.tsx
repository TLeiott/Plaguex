...
import { Pause, Play, Trash2, Download, ChevronDown, ExternalLink, Smartphone } from 'lucide-react'
        ...
        <>
          {platform().externalPlayer ? (
            <Link
              to="/play/$ratingKey"
              params={{ ratingKey: d.item.ratingKey }}
              search={{ offline: true, external: !externalDefault }}
              className="rounded-lg p-2 text-fg-2 hover:bg-bg-3 hover:text-fg"
              aria-label={externalDefault ? 'Play in app' : 'Play in another app'}
              title={externalDefault ? 'Play in app' : 'Play in another app'}
